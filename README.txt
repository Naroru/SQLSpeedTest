...
Запросы исполняются в рамках сессии.
Жизнь сессии, если ты не вручную её менеджишь, равна жизни транзакции. Поэтому запросы в цикле, обратаываемые в рамках одной транзакции, быстрее.
...
Замеры скорости реализованы в классах - тестах
--------------------------------------------------
...
ВЫБОРКА
Выборка через JDBC Template не в цикле работает быстрее, чем через Репозиторий. При этом даже выборка в цикле через JDBC Template отрабатывает быстрее
...
из-за создания множества соединений, что можно поправить аннотацией @Transactional
